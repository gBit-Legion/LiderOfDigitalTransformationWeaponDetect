...
class DatasetSpliter:
    ...
    def move_files(self, files):
        ...
        for file in files:
            ...
            source_label_path = os.path.join(self.source_folder, "labels", file.replace(".jpg", ".txt").replace(".jpeg", ".txt").replace(".png", ".txt").replace(".webp", ".txt").replace(".JPG", ".txt"))
            destination_image_path = os.path.join(self.destination_folder, "images", file)
            destination_label_path = os.path.join(self.destination_folder, "labels", file.replace(".jpg", ".txt").replace(".jpeg", ".txt").replace(".png", ".txt").replace(".webp", ".txt").replace(".JPG", ".txt"))
